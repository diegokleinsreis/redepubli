...
$post_id_to_toggle = isset($_GET['id']) ? (int)$_GET['id'] : 0;
$denuncia_id = isset($_GET['denuncia_id']) ? (int)$_GET['denuncia_id'] : 0;

if ($post_id_to_toggle > 0) {
    $sql_check = "SELECT status FROM Postagens WHERE id = ?";
    ...
    if ($post = $result->fetch_assoc()) {
        $new_status = ($post['status'] === 'ativo') ? 'inativo' : 'ativo';
...
if ($denuncia_id > 0) {
    ...
    header("Location: ../../admin/admin_postagens.php");
}
exit();
?>